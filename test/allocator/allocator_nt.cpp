...
Test(default_nt_allocator_test)
{
	TestInit(default_nt_allocator_test);

	Testcase(test_allocate_requirements)
	{
		AssertTrue(ds::is_same<decltype(ds::default_nt_allocator::allocate(size_t(0), ds::align_t(0))), void *>::value);
	} TestcaseEnd(test_allocate_requirements);

	Testcase(test_deallocate_requirements)
	{
		AssertTrue(ds::is_same<decltype(ds::default_nt_allocator::deallocate((void *)nullptr)), void>::value);
	...
	Testcase(test_failing_allocation)
	{
		...
		void * block_ = nullptr;
		AssertThrowNone(block_ = ds::default_nt_allocator::allocate(size_));
		ExpectNull(block_);
		ds::default_nt_allocator::deallocate(block_);
...
TestRegistry(default_nt_allocator_test)
{
...
int main()
{
	return default_nt_allocator_test().run_all(reporter_t<default_nt_allocator_test>(pptest::normal));
}